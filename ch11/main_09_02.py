...
def pre_model_guardrail_node(state: AgentState):
    result = pre_model_guardrail(state)

    # guardrail triggered → refuse and stop
    if "llm_input_messages" in result:
        refusal_response = llm_model.invoke(result["llm_input_messages"])

        return Command(
            update={"messages": [refusal_response]},
            goto=END
        )

    # guardrail passed → continue normally
    return Command(update=state, goto="llm_node")
...
builder.add_node("guardrail", pre_model_guardrail_node)

builder.add_conditional_edges("guardrail", lambda x: "llm_node")
...
booking_builder.add_node("guardrail", pre_model_guardrail_node)

booking_builder.add_conditional_edges("guardrail", lambda x: "llm_node")
booking_builder.add_conditional_edges("llm_node", route_tools)

booking_builder.add_edge("tools", "llm_node")
booking_builder.set_entry_point("guardrail")
    ...
    if isinstance(last_msg, HumanMessage):
        ...
        if not decision.is_travel:
            refusal_text = (
                ...
                lodging, prices, availability, or weather in Cornwell/England).
                Please rephrase you request to be travel-related."""
            )
...
class GuardrailDecision(BaseModel):
    is_travel: bool = Field(
        ...,
        description = ("""True if the user question is about travel information:
                       destinations, attractions, lodging (hotels/BnBs), prices, availability, or 
                       weather in Cornwall/England."""
        ),
    )
    reason: str  = Field(
        ...,
        description = ("Bried justification for the decision.")

    )

GUARDRAIL_SYSTEM_PROMPT = (
    """You are a strict classifier, Given the users's last message, respond with whether it is
    travel-related. Travel-related queries include destinations, attractions, lodging (hotels/BnBx),
...
REFUSAL_INSTRUCTIONS = (
    ...
    or weather in Cornwall/England). The user's request is not travel-related. Politely refuse and 
